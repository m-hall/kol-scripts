...
// @author         Hellion
// @author         Aelsa
// @author	Buzzy (autoupdate function; see https://userscripts-mirror.org/scripts/show/52251 )
...
var adventureChoiceNumber = 0,
    SpoilerSet, imageName;
var n = 0,
    sp_list = "";
var debug = false;
...
if (window.name == "mainpane") {
    if (inputs.length === 0) {
        return;
    } //not in a place where we need to do anything.
    ...
    if (SpoilerSet === undefined || SpoilerSet === null) {
        ...
        for (var foo = 0; foo < images.length; foo++) {
            if (images[foo].src.indexOf('adventureimages') != -1) {
                imageName = images[foo].src.split('/')[4];
                break;
            }
        }
...
function CheckButtonText(inputs, cNum) {
    var advOptions = {
        "594": {
            "check out the mini-fridge": "enable glasses or pill bottle",
            "turn on the tv": "enable glasses or comb",
            "take a nap": "enable pill bottle or comb",
            "pick up the glasses": "\nafter fridge and tv: acquire glasses",
            "pick up the comb": "\nafter nap and tv: acquire unbreakable comb",
            "open the pill bottle": "\nafter fridge and nap: acquire lost pill bottle",
            "walk out and back in": "keep trying",
            "walk into the room": "get started"
        },
        "524": {
            "table of contents": "display chapter list",
            "skip to your reward": "acquire skullheads's screw",
            "quit reading": "leave"
        },
        "546": {
            "visit vlad's boutique": "proceed to vlad (acquire an effect (+weapon damage / +spell damage / +DR)",
...
function Describe(el, description) {
    ...
    if (cval === -1) { // got here without setting a button value? not a Choice.php button set.
        ...
        for (n = 0; n < inputs.length; n++) {
            if (inputs[n].type === "submit") {
                Describe(inputs[n], SpoilerSet[cval++]);
...
function GetSpoilersForAdvNumber(advNumber) {
    ...
    if (advOptions[advNumber] !== undefined) {
        return advOptions[advNumber];
    } else {
        return null;
    }
}
